...
public class XpressionMapperVisitor : ExpressionVisitor
{
    ...
    protected override Expression VisitMember(MemberExpression node)
    {
        ...
        {
            Expression mappedParentExpression = Visit(parentExpression);
            ...
            if (propertyMapInfoList.Any(x => x.CustomExpression != null))
            {
                ...
                TypeMappings.AddTypeMapping(ConfigurationProvider, node.Type, fromCustomExpression.Type);
            ...
            TypeMappings.AddTypeMapping(ConfigurationProvider, node.Type, memberExpression.Type);
    ...
    {
        var ex = Visit(node.Body);

        var mapped = Expression.Lambda(TypeMappings.ReplaceType(node.Type), ex, node.GetDestinationParameterExpressions(InfoDictionary, TypeMappings));
        TypeMappings.AddTypeMapping(ConfigurationProvider, node.Type, mapped.Type);
        return mapped;
    }
    ...
    protected override Expression VisitNew(NewExpression node)
    {
        ...
        else if (node.Arguments.Count > 0 && IsAnonymousType(node.Type))
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
                bindingExpressions.Add(parameters[i].Name, Visit(node.Arguments[i]));
            ...
            (
                node.Bindings
                    ...
                    (
                        ...
                        binding => Visit(binding.Expression)
                    ),
                node.Type
            );
        }
    ...
    private MemberBinding DoBind(MemberInfo sourceMember, Expression initial, Expression mapped)
    {
        ...
        TypeMappings.AddTypeMapping(ConfigurationProvider, initial.Type, mapped.Type);
    ...
    {
        return DoVisitBinary(Visit(node.Left), Visit(node.Right), Visit(node.Conversion));
    ...
    {
        return DoVisitConditional(Visit(c.Test), Visit(c.IfTrue), Visit(c.IfFalse));
    ...
    {
        if (TypeMappings.TryGetValue(node.TypeOperand, out Type mappedType))
            return MapTypeBinary(Visit(node.Expression));
    ...
    protected override Expression VisitUnary(UnaryExpression node)
    {
        ...
        {
            if (TypeMappings.TryGetValue(node.Type, out Type mappedType))
    ...
    {
        Type newType = TypeMappings.ReplaceType(node.Type);
        if (newType != node.Type)
        {
            ...
            if (typeof(Expression).IsAssignableFrom(node.Type))
                return Expression.Constant(Visit((Expression)node.Value), newType);
        }
    ...
    protected override Expression VisitMethodCall(MethodCallExpression node)
    {
        ...
        var typeArgsForNewMethod = node.Method.IsGenericMethod
            ? node.Method.GetGenericArguments().Select(type => TypeMappings.ReplaceType(type)).ToList()//not converting the type it is not in the typeMappings dictionary
            : null;
        ...
        return node.Method.IsStatic
            ? GetStaticExpression()
            : GetInstanceExpression(Visit(node.Object));
