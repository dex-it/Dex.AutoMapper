...
public abstract class EntityFramworkCoreTestsBase : IDisposable
{
    ...
    public async Task Persist_InsertOrUpdateAsync_WhenEntityExist_ThenTheEntityShouldBeInTheModifiedState()
    {
        ...
        var item = await db.Things.FirstAsync(cancellationToken: TestContext.Current.CancellationToken);

        // Act
        await db.Things.Persist(mapper).InsertOrUpdateAsync(new ThingDto { ID = item.ID, Title = "Test" }, TestContext.Current.CancellationToken);
    ...
    public async Task Persist_InsertOrUpdateAsync_WhenEntityExist_ThenTheEntityShouldBeUpdated()
    {
        ...
        // Assert
        (await db.Things.CountAsync(cancellationToken: TestContext.Current.CancellationToken)).Should().Be(3);
        (await db.Things.FirstOrDefaultAsync(x => x.ID == item.ID, cancellationToken: TestContext.Current.CancellationToken)).Title.Should().Be("Test");
    }
    ...
    public async Task Persist_InsertOrUpdateAsync_WhenEntityDoesNotExist_ThenTheEntityShouldBeInTheAddedState()
    {
        ...
        // Act
        var createdThing = await db.Things.Persist(mapper).InsertOrUpdateAsync(new ThingDto { Title = "Test" }, TestContext.Current.CancellationToken);
    ...
    public async Task Persist_InsertOrUpdateAsync_WhenEntityDoesNotExist_ThenTheEntityShouldBeInserted()
    {
        ...
        // Assert
        createdThing.Should().NotBeNull();
        (await db.Things.CountAsync(cancellationToken: TestContext.Current.CancellationToken)).Should().Be(4);
        var createdThingFromEF = await db.Things.OrderByDescending(x => x.ID).FirstOrDefaultAsync(cancellationToken: TestContext.Current.CancellationToken);
        createdThingFromEF?.Title.Should().Be("Test");
        createdThing.Should().BeEquivalentTo(createdThingFromEF);
    }
    ...
    public async Task Persist_RemoveAsync_WhenEntityExist_ThenTheEntityShouldBeInTheDeletedState()
    {
        ...
        // Act
        await db.Things.Persist(mapper).RemoveAsync(new ThingDto { ID = item.ID, Title = "Test" }, TestContext.Current.CancellationToken);
    ...
    public async Task Persist_RemoveAsync_WhenEntityExist_ThenTheEntityShouldBeDeleted()
    {
        ...
        // Assert
        (await db.Things.CountAsync(cancellationToken: TestContext.Current.CancellationToken)).Should().Be(2);
        (await db.Things.FindAsync(new object[] { item.ID }, TestContext.Current.CancellationToken)).Should().BeNull();
    }
    ...
    public async Task Persist_RemoveAsync_WhenEntityDoesNotExist_ThenShouldBeUnchangedState()
    {
        ...
        // Act
        await db.Things.Persist(mapper).RemoveAsync(new ThingDto { Title = "ignored" }, TestContext.Current.CancellationToken);
    ...
    public async Task Persist_RemoveAsync_WhenEntityDoesNotExist_ThenShouldBeNoChange()
    {
        ...
        var item = await db.Things.OrderByDescending(x => x.ID).FirstAsync(cancellationToken: TestContext.Current.CancellationToken);
        ...
        // Assert
        (await db.Things.CountAsync(cancellationToken: TestContext.Current.CancellationToken)).Should().Be(3);
        (await db.Things.FindAsync(new object[] { item.ID + 1 }, TestContext.Current.CancellationToken)).Should().BeNull();
    }
    ...
    public async Task Persist_InsertOrUpdateAsync_WhenSameEntity_ThenTheEntityShouldBeInTheUnchangedState()
    {
        ...
        var item = await db.Things.Where(x => x.Title == "Test3").FirstAsync(cancellationToken: TestContext.Current.CancellationToken);

        // Act
        await db.Things.Persist(mapper).InsertOrUpdateAsync(new ThingDto { ID = item.ID, Title = item.Title }, TestContext.Current.CancellationToken);
    ...
    public async Task Persist_InsertOrUpdateAsync_WhenSameEntity_ThenSavedEntityShouldBeSame()
    {
        ...
        // Assert
        (await db.Things.CountAsync(cancellationToken: TestContext.Current.CancellationToken)).Should().Be(3);
        (await db.Things.FirstOrDefaultAsync(x => x.ID == item.ID, cancellationToken: TestContext.Current.CancellationToken))?.Title.Should().Be(item.Title);
    }
